...
if __name__ == '__main__':
    ...
    for fidx in range(n_files):
        ...
        # paste track emb
        if IS_TRACK_EMB_FULL:
            prev = x[0][0]
            for i in range(1, len(x)):
                if word2event['bar-beat'][x[i][3]] == 'Bar':
                    continue
                elif word2event['track'][x[i][0]] != 0:
                    prev = x[i][0]
                else:
                    x[i][0] = prev
